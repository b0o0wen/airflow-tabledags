...
# 有mockdate则取mockdate，无此参时取当前date
...
END_DATE=`date -d "$TODAY -2 days" '+%Y%m%d'`
FLAG_DATE=`date -d "$TODAY -2 days" '+%Y%m%d'`
...
IS_SUCCESS=`cat {{log_path}} | grep -E 'die|error|fail'  | wc -l`
        ...
        then
            STOP_DATE_14=`date -d "$START_DATE +15 days" "+%Y%m%d"`
            echo 'STOP_DATE_14' $STOP_DATE_14
                if [ $END_DATE -lt $STOP_DATE_14 ]
                then 
